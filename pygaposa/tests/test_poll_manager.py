...
@pytest.fixture
def poll_manager(mock_poll, mock_logger):
...
async def test_wait_for_update(poll_manager, mock_poll):
    await poll_manager.wait_for_update()
    mock_poll.assert_awaited_once()


async def test_wait_for_condition(poll_manager, mock_poll):
...
async def test_wait_for_condition_with_timeout(poll_manager, mock_poll):
...
async def test_wait_for_condition_three_times(poll_manager, mock_poll):
